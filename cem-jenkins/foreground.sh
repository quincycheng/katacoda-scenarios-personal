...
timeout 120s bash -c 'while [ "$(docker image ls | grep jenkins/jenkins)" = "" ];do printf ".";sleep 2s;done'   && \
printf "✅\n- Setting up Jenkins..." && \
timeout 120s bash -c 'while [ "$(docker ps -a|grep jenkins)" = "" ];do printf ".";sleep 2s;done'   && \
echo -e "✅\n- Ready! 😀"